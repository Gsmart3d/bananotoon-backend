...
function loadEndpointsMapping() {
  ...
  try {
    ...
    console.log(`✅ Loaded ${endpointsMapping.endpoints.length} endpoint configurations`);
    return endpointsMapping;
  } catch (error) {
    console.error('❌ Failed to load endpoints mapping:', error.message);
    return { endpoints: [] };
  }
}
...
function findEndpointForModel(modelId) {
  ...
  if (!endpoint) {
    console.warn(`⚠️ No endpoint configuration found for model: ${modelId}`);
    return null;
  }

  console.log(`✅ Found endpoint for ${modelId}: ${endpoint.endpoint}`);
  return endpoint;
}
